...
function getMeta(html:string,key:string){
 for(const tag of html.match(/<meta[^>]*>/gi)||[]){
  if(tag.includes('property="'+key+'"')||tag.includes('name="'+key+'"')||tag.includes("property='"+key+"'")||tag.includes("name='"+key+"'")){
   const m=tag.match(/content=["']([^"']*)["']/i);if(m)return clean(m[1]);
  }
 }
 return "";
}
...
function imageCandidates(html:string,base:URL){
 ...
 const add=(raw:string)=>{
  const v=raw.replace(/\\u002F/g,"/").replace(/\\u0026/g,"&").replace(/\\\//g,"/");
 ...
 const quoted=/["']((?:https?:)?\/\/[^"'<>\s]+?\.(?:jpg|jpeg|png|webp)(?:\?[^"'<>\s]*)?)["']/gi;
 for(const m of html.matchAll(quoted)){add(m[1]);if(out.length>=20)break}
...
function priceFrom(html:string){
 ...
 for(const re of [/"(?:salePrice|minPrice|price)"\s*:\s*"?([0-9]+(?:\.[0-9]+)?)/i,/US\s*\$\s*([0-9]+(?:\.[0-9]+)?)/i]){const m=html.match(re);if(m)return Number(m[1])}
 return null;
}
