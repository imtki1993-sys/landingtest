...
export async function GET(req:Request){try{
 ...
 return NextResponse.json({totals:{landings:landings.length,published:landings.filter(x=>x.status==="PUBLISHED").length,products:productsQ.count||0,orders:totalOrders,revenue:Number(t.revenue||0),deliveredRevenue:Number(t.deliveredRevenue||0),views,conversion:views?totalOrders/views*100:0,confirmationRate:totalOrders?confirmed/totalOrders*100:0,deliveryRate:shipped?delivered/shipped*100:0,newLeads:leads.filter(x=>x.status==="NEW").length,confirmed,periodDays:30},recentOrders:orders.map(o=>({...o,lead:lm.get(o.lead_id)||null,product:pm.get(o.product_id)||null,landing:pgm.get(o.landing_page_id)||null})),topLandings:(Array.isArray(a.landings)?a.landings:[]).slice(0,5)});
