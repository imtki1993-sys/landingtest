...
const PUBLIC_GET=["/login","/api/domain-resolve","/api/landing/","/landing/","/_next/","/favicon.ico"];
function isPublic(req:NextRequest){const p=req.nextUrl.pathname;if(PUBLIC_GET.some(x=>p===x||p.startsWith(x)))return true;if(p==="/api/orders"&&req.method==="POST")return true;if(p==="/api/track"&&req.method==="POST")return true;if(p==="/api/auth/login"||p==="/api/auth/logout")return true;return false}
